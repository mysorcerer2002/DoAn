"""Integration tests cho Partner API: register, admin approve, list my partners, get partner me."""
...
async def test_admin_approve_tenant(client: AsyncClient, db_session):
    ...
    partner_id = reg_resp.json()["id"]

    resp = await client.post(
        f"/admin/partners/{partner_id}/approve",
        json={"approve": True},
        headers=_auth_headers(admin.id),
    )
    assert resp.status_code == 200
...
# ── GET /users/me/partners-as-staff ──
...
async def test_list_my_tenants(client: AsyncClient, db_session):
    ...
    data = resp.json()
    assert len(data) == 2
    # Mỗi item phải có structure PartnerStaffSummary
    for item in data:
        assert "id" in item
        assert "name" in item
        assert "slug" in item
...
async def test_get_tenant_me_active(client: AsyncClient, db_session):
    ...
    resp = await client.get(
        "/partners/me",
        headers={**_auth_headers(owner.id), "X-Partner-Id": str(partner_id)},
    )
    assert resp.status_code == 200
    assert resp.json()["status"] == "active"
...
async def test_get_tenant_me_not_found(client: AsyncClient, db_session):
    ...
    resp = await client.get(
        "/partners/me",
        headers={**_auth_headers(owner.id), "X-Partner-Id": "9999"},
    )
    # Sau khi thêm require_staff_in_partner, non-member luôn 403 trước khi check 404
    assert resp.status_code == 403
...
async def test_get_tenant_me_non_member_forbidden(client: AsyncClient, db_session):
    ...
    partner_a_id = reg_a.json()["id"]
    await client.post(
        f"/admin/partners/{partner_a_id}/approve",
        json={"approve": True},
        headers=_auth_headers(admin.id),
    )
    ...
    partner_b_id = reg_b.json()["id"]
    await client.post(
        f"/admin/partners/{partner_b_id}/approve",
        json={"approve": True},
        headers=_auth_headers(admin.id),
    )
    ...
    resp = await client.get(
        "/partners/me",
        headers={**_auth_headers(owner_a.id), "X-Partner-Id": str(partner_b_id)},
    )
    assert resp.status_code == 403


async def test_patch_tenant_me_owner_can_update(client: AsyncClient, db_session):
    """I3 fix: PATCH /partners/me cho owner update info."""
    admin = await _create_user(
