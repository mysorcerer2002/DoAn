...
class EnrollFormInput(BaseModel):
    ...
    @classmethod
    def _ends_at_in_future(cls, v: datetime) -> datetime:
        # Voucher service ở sign_and_enroll set authorization.valid_until =
        # ends_at + 90 ngày; nếu ends_at nằm trong quá khứ thì valid_until có
        # thể vi phạm CHECK `ck_tenant_authorizations_valid_window`
        # (valid_until > valid_from ≈ NOW). Ép ends_at phải là tương lai.
        now = datetime.now(UTC)
        # Cho phép naive datetime (FE có thể gửi ISO không kèm TZ) — coi như UTC.
        v_aware = v if v.tzinfo else v.replace(tzinfo=UTC)
        if v_aware <= now:
            raise ValueError(
                "ends_at phải là thời điểm tương lai (sau hiện tại)"
            )
        return v
