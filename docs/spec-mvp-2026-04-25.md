...
**KHÔNG drop `transactions.voucher_id`, `voucher_discount_amount`, `legal_discount_ratio`** — giữ orphan trong schema để đồng bộ với "KEEP campaign infra cho thesis" (xem dưới). Runtime MVP set `voucher_id=NULL`, hai cột còn lại NULL → GENERATED `legal_discount_ratio` cũng NULL.
...
| `notifications` | Không có FK với campaign infra; không có MVP feature push notification |

### Bảng GIỮ orphan trong DB (Path A — reserved cho luận văn tốt nghiệp)

User chốt 2026-04-25: 3 bảng `campaign_regulatory_submissions`, `campaign_issuances`, `campaign_approval_events` reserve cho luận văn (managed service NĐ 81). FK chain RESTRICT buộc phải giữ luôn các bảng cha:

| Bảng giữ | Lý do FK chain |
|---|---|
| `campaign_regulatory_submissions` | **User explicit reserve cho thesis** |
| `campaign_issuances` | **User explicit reserve cho thesis** |
| `campaign_approval_events` | **User explicit reserve cho thesis** |
| `campaigns` | 3 bảng trên FK RESTRICT tới `campaigns.id` |
| `campaign_templates` | `campaigns.template_id FK RESTRICT` |
| `vouchers` | `vouchers FK campaigns + campaign_issuances` (RESTRICT) |
| `partner_authorizations` | `partner_authorizations FK campaigns RESTRICT` (đối xứng) |
| `transactions.voucher_id` (cột) | `FK vouchers SET NULL` — giữ cột để FK chain hợp lệ |

**Trade-off**:
- ✅ Thesis bật lại managed service: chỉ add lại services/routes/FE, KHÔNG cần Alembic migration mới
- ✅ Pivot migration đơn giản hơn (giảm 7 drop_table)
- ❌ ~7 bảng dead schema trong production DB MVP
- ❌ Codebase người đọc thấy model file tồn tại nhưng không có route/service tương ứng

**Runtime MVP**:
- Routes/services của campaign/voucher/notification → DROP hoàn toàn (xem §7)
- Model files KEEP orphan trên disk (baseline migration import — nếu xoá sẽ ImportError khi wipe-and-reseed)
- FE hooks/pages của campaign/voucher → DROP
- Báo cáo đồ án: ẩn các bảng này khỏi sơ đồ class (ghi ở "Future Work")
...
- `transactions` — runtime MVP dùng `{id, partner_id, membership_id, staff_id, gross_amount, net_amount, points_earned, method, note, receipt_code}`. Cột orphan `voucher_id, voucher_discount_amount, legal_discount_ratio` GIỮ trong schema (Path A) nhưng service luôn set NULL. Membership_id giữ vì transaction luôn at-shop.
...
```python
def upgrade():
    # Path A: KEEP campaign infra (campaigns, campaign_templates, vouchers, partner_authorizations,
    # campaign_issuances, campaign_regulatory_submissions, campaign_approval_events) orphan trong DB
    # cho luận văn. KHÔNG drop view v_campaign_stats vì campaigns table còn.

    # 1. Drop bảng không có FK chain với campaign infra
    op.drop_table("notifications")              # FK chỉ tới partners + users, không vướng campaigns
    op.drop_table("partner_staff")              # 1 account = 1 shop
    op.drop_table("partner_settings_audit")     # audit log settings, không cần MVP

    # 2. Drop cột tier trong các bảng giữ
    #    LƯU Ý: KHÔNG drop transactions.{voucher_id, voucher_discount_amount, legal_discount_ratio}
    #    — giữ orphan để FK chain với vouchers còn hợp lệ (Path A). Service set NULL khi insert.
...
### Backend models — phân loại 2 nhóm

Baseline migration `c4d5e6f7a8b9_create_loyalty_baseline_schema.py` import toàn bộ model files để `Base.metadata.create_all` register schema. Wipe-and-reseed sẽ re-run từ migration đầu — nếu xoá file Python → `ImportError` chặn container start.

→ **Phương án (a) — KEEP TẤT CẢ model files trên disk**, không xoá file `.py`. Khác biệt là semantic:

**Nhóm A — model "live" (table tồn tại + Path A keep cho thesis)**:

| File model | Table DB | Status runtime |
|---|---|---|
| `app/models/campaign.py` | `campaigns` | Schema only, không có route/service MVP |
| `app/models/campaign_template.py` | `campaign_templates` | Schema only |
| `app/models/voucher.py` | `vouchers` | Schema only |
| `app/models/campaign_issuance.py` | `campaign_issuances` | Schema only |
| `app/models/campaign_regulatory_submission.py` | `campaign_regulatory_submissions` | Schema only |
| `app/models/campaign_approval_event.py` | `campaign_approval_events` | Schema only |
| `app/models/partner_authorization.py` | `partner_authorizations` | Schema only |

→ Xoá khỏi `app/models/__init__.py` exports (nếu có) để runtime app code không nhỡ tay import. Schema vẫn register vào `Base.metadata` qua module-level class definition.

**Nhóm B — model "orphan" (table đã DROP nhưng file giữ vì baseline import)**:

| File model | Table DB | Status |
|---|---|---|
| `app/models/tier.py` | (dropped) | Class còn trong metadata, table không có trong DB sau pivot. App không query. |
| `app/models/partner_staff.py` | (dropped) | Tương tự |
| `app/models/partner_settings_audit.py` | (dropped) | Tương tự |
| `app/models/notification.py` | (dropped) | Tương tự |

→ Cùng strategy: KHÔNG xoá file, xoá exports, xoá routes/services. SQLAlchemy không complain khi metadata register table không tồn tại ở runtime miễn app không query.
