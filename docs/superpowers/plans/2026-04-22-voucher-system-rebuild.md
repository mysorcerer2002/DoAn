...
**Atomic claim giữ nguyên**: `UPDATE campaigns WHERE issued_count < max_issuances` trước đây thay bằng:
```sql
-- Dùng advisory lock OR subquery COUNT check trong atomic transaction:
...
-- raise nếu affected_rows = 0
```
+ `AFTER INSERT ON vouchers` trigger **chỉ validate** (không mutate): raise nếu vượt `max_issuances` — backup check, không phải source of truth.
...
| status | ENUM(draft, invoiced, paid, waived) | | |
...
**Partial unique** `(campaign_id, fee_type) WHERE status != 'waived'` — chặn double-charge fee cùng loại cho cùng campaign (C4).
...
approve(campaign_id):
  ...
  assert campaign.service_fee_status IN ('paid','none')
  ...
  -- chỉ đủ 4 điều kiện trên mới approve được
```
...
| M11 | `029_alter_campaigns_add_authorization_fee_fields.py` | `authorization_id`, `service_fee_total`, `service_fee_status`, `ops_filing_started_at` | DROP |
...
**`CampaignApprovalService.reject` (C3):**
```python
def reject(campaign_id, admin_user_id, reason):
    ...
    if used_count > 0:
        # Return warning to admin UI; reject vẫn proceed; voucher đã used giữ nguyên
    ...
    # Notify members (nếu notification module có)
    # Event log
...
CAMPAIGN_AUTO_THRESHOLD: int = 1_000_000
CAMPAIGN_NOTIFY_THRESHOLD: int = 10_000_000
...
1. **Threshold số tiền**: `CAMPAIGN_AUTO_THRESHOLD=1M` / `NOTIFY_THRESHOLD=10M`. OK cho scope đồ án hay dùng số nhỏ hơn (100k / 500k) để dễ demo trigger các tier?
2. **Bảng giá fee mặc định**: `so_ct_filing=500k, dossier_preparation=1M, multi_province=+2M, express=+500k, waiver=0`. OK?
3. **Post-report 45-day job**: implement v1 hay defer?
4. **Retention purge job**: implement v1 (weekly) hay defer + chỉ tạo column `retention_until` thôi?
5. **Rendered PDF văn bản uỷ quyền**: v1 chỉ render HTML + hash; hay muốn render PDF thật (lib weasyprint) cho demo đẹp?
6. **Notification khi voucher bị cancel**: v1 log-only (không push), hay implement minimal in-app notification?
7. **Seed demo**: tạo sẵn 3 shop × 3 template × 2 campaign (1 auto / 1 pending) để demo flow ngay không phải click tạo?
...
1. User answer 7 items mục 15.
2. User approve execute v2.
3. Em gọi `superpowers:code-reviewer` pass 2 (optional — nếu user muốn verify fix đã áp dụng đúng).
4. Execute phase 1.
