...
| **5** | **Pre-flight: chạy 4 SQL verify ở §4.1**, fix orphan nếu có. M5 migration + add user.points_balance + membership rename + ledger refactor + reward expand + voucher_template create. **Post-flight: SQL invariant `users.points_balance == SUM(point_ledger.delta)`** | Pre-flight 4 query: orphan_ledger=0, orphan_redemption=0, balance khớp. Post: invariant query trả empty. Backend boot OK; smoke POS earn | **One-way migration** (M5 `downgrade()` raise `NotImplementedError` — drop column + scope rename không reversible an toàn; rollback = restore DB snapshot trước M5) |
