...
1. **A. Service fee removal (clean-break, scope hẹp):**
   - Xoá 2 bảng DB `campaign_service_fees`, `campaign_fee_schedules`.
   - Xoá trên `campaigns`: **2 cột** `service_fee_total`, `service_fee_status` + **2 check constraint** `ck_campaigns_service_fee_total_nonneg`, `ck_campaigns_service_fee_status`.
   - **GIỮ** `campaigns.authorization_id` + FK `fk_campaigns_authorization_id` + bảng `partner_authorizations` — các artifact này thuộc **managed service model** (luồng uỷ quyền công ty ops thay shop nộp hồ sơ Sở CT), KHÔNG phải service fee. `voucher_service.py` (L.142, 313, 331) + `transaction_service._maybe_issue_welcome_voucher` (L.546, 557) đang phụ thuộc `authorization_id` cho guard "uỷ quyền đã thu hồi không phát voucher". Drop sẽ vỡ runtime. Memory `project_managed_service_model.md` xác nhận managed service là core, fee chỉ là monetization layer bị disable.
   - Xoá code: models/schemas/services/API/jobs/frontend types+hooks+UI tham chiếu **service_fee / ServiceFee / FeeSchedule** (không chạm authorization).
   - Xoá flag `settings.service_fee_enabled` khỏi `config.py`.
   - Sau merge: `grep -r "service_fee\|ServiceFee\|FeeSchedule" backend/app/ frontend/src/` trả về **0 match** (chỉ còn trong `alembic/versions/` của historical migrations — không touch để giữ migration chain valid). Các occurrence `authorization_id` / `PartnerAuthorization` giữ nguyên.
...
| `backend/app/models/campaign.py` | Remove fields `service_fee_total`, `service_fee_status`; remove 2 check constraints `ck_campaigns_service_fee_total_nonneg`, `ck_campaigns_service_fee_status` (L.99-105 constraints, L.199-205 columns). **GIỮ** `authorization_id` + FK. |
| `backend/app/services/campaign_fee_service.py` | **Delete file** (2.8K, service chính của fee preview / invoice logic). |
| `backend/app/schemas/partner_authorization.py` | File chứa schema cho BOTH `partner_authorization` (giữ) VÀ `campaign_service_fee` (xoá). Action: xoá class `CampaignServiceFeeResponse` + rewrite module docstring chỉ còn authorization; giữ các schema authorization lại. |
...
| `backend/app/services/campaign_enrollment_service.py` | Remove fee preview logic (L.247-257 `_build_fee_preview`), remove `service_fee_enabled` branch, remove imports `CampaignServiceFee`, `CampaignFeeSchedule`, `FeeStatus`, `CampaignFeeService` |
...
| `backend/app/jobs/purge_retention.py` | **CHỐT GIỮ FILE** — job vẫn còn nhiệm vụ `DELETE FROM partner_authorizations WHERE expired_at < now() - interval` (L.58). Actions: (1) xoá SQL DELETE cho `campaign_service_fees` (L.~67), (2) xoá log line "hard-delete %d campaign_service_fees" (L.84), (3) đơn giản hoá return value về chỉ `auth_deleted`, (4) update docstring bỏ nhắc `campaign_service_fees`. KHÔNG delete file. KHÔNG gỡ APScheduler registration. |
| `backend/app/core/config.py` | Remove `service_fee_enabled: bool = False` (L.36-38) + docstring comment |
| `backend/app/main.py` | **Thêm branch** nhận diện constraint `ux_transactions_partner_receipt_code` trong IntegrityError handler (L.99-118) → map sang message VN `"Mã hoá đơn đã tồn tại, vui lòng dùng mã khác."` (phục vụ E6 của Part C; đặt trong A scope vì cần sửa cùng file). |
...
- `upgrade()` (thứ tự quan trọng: drop check → drop column → drop indexes → drop table):
  ...
  3. `op.drop_column("campaigns", "service_fee_status")`
  4. `op.drop_column("campaigns", "service_fee_total")`
  5. `op.drop_index("ix_campaign_service_fees_partner_status", table_name="campaign_service_fees")`
  6. `op.drop_index("ux_campaign_service_fees_active_per_type", table_name="campaign_service_fees")`
  7. `op.drop_table("campaign_service_fees")`
  8. `op.drop_index("ux_campaign_fee_schedules_active_per_type", table_name="campaign_fee_schedules")`
  9. `op.drop_table("campaign_fee_schedules")`

  **KHÔNG drop** `fk_campaigns_authorization_id` và `authorization_id` — thuộc managed service scope (xem 2.1 goal 1).
- `downgrade()`:
   - **Quyết: `pass` + docstring comment rõ ràng.**
   ```python
   def downgrade() -> None:
       """One-way migration — không restore được service fee schema.

       Nếu thật sự cần rollback tới trước revision này: checkout commit
       trước merge spec + run migration chain theo đường cũ (alembic
       downgrade tới revision `a3b4c5d6e7f8` sẽ vô hại vì đường cũ có
       sẵn các migration a4/a6/a7 tạo lại 2 bảng + 3 cột).

       Viết downgrade thực (150 dòng copy từ 3 historical revision) gây
       trùng code + rủi ro drift nếu historical revision bị chỉnh — giữ
       no-op là safe cho CI round-trip test (alembic downgrade không crash).
       """
       pass
   ```
   - **Lý do ưu tiên `pass` over `NotImplementedError`:**
     1. CI pipeline chạy round-trip test `alembic upgrade head` → `alembic downgrade -1` → `alembic upgrade head` để validate migration chain — raise exception sẽ fail build.
     2. Production rollback hiếm; nếu cần thì revert commit + deploy lại (full path migration), không dùng `alembic downgrade` trực tiếp.
   - **Trade-off:** sau khi merge spec này, `alembic downgrade -1` sẽ đi qua migration nhưng KHÔNG restore schema — developer thấy warning "table không tồn tại" khi query kiểm tra. Document trong docstring đủ để cảnh báo.
...
**`TransactionService._calculate_points` (backend/app/services/transaction_service.py L.236-241):**

```python
# Trước (chữ ký hiện tại — @staticmethod, SYNC, không biết membership):
@staticmethod
...
```python
# Sau (thêm kwarg `membership`, vẫn SYNC, vẫn @staticmethod):
@staticmethod
def _calculate_points(
    ...
    *,
    ...
    """Tính điểm từ `amount` (có thể là gross hoặc net — caller quyết).
    Nếu `rule.use_tiers=True` và `membership.current_tier_id` có giá trị,
    nhân thêm hệ số `tier.earn_multiplier`. Tier object phải được caller
    load sẵn qua `selectinload(Membership.current_tier)` để tránh lazy
    load async context.
    """
...
**Ghi chú signature:**
- Giữ `@staticmethod` + SYNC — tier object KHÔNG lookup trong hàm này (sẽ gây async-in-sync); caller phải eager-load `membership.current_tier` qua `selectinload` khi fetch membership (caller update: xem dưới).
- Rename `net_amount` → `amount` vì call site `create_manual` có thể pass `gross_amount` (settings `points_on_gross`). Bản chất parameter là "số tiền dùng để tính điểm", không nhất thiết là net.
- `membership` mặc định `None` để backward-compatible với test fixture gọi trực tiếp `_calculate_points(rule, 100_000)` (nếu có). Nếu `None` → multiplier = 1.00 (giữ hành vi cũ).

**Hai call site — cập nhật cả hai:**

| # | File:Line | Hiện tại | Đổi thành | Ghi chú |
|---|-----------|----------|-----------|---------|
| 1 | `transaction_service.py:168` (`create_manual`) | `self._calculate_points(rule, amount_for_points)` | `self._calculate_points(rule, amount_for_points, membership=membership)` | `amount_for_points = gross_amount if points_on_gross else net_amount` — logic này GIỮ NGUYÊN. Multiplier áp dụng SAU khi chọn gross/net. |
| 2 | `transaction_service.py:416` (`_create_transaction_for_membership`, tức flow QR + scheduled + POS membership) | `self._calculate_points(rule, net_amount)` | `self._calculate_points(rule, net_amount, membership=membership)` | Flow này hiện KHÔNG honor `points_on_gross` (latent bug phát hiện qua review). **Scope spec này KHÔNG fix bug đó** — chỉ thêm multiplier. Ghi chú TODO trong code + mention ở phụ lục Khoá luận là đủ. |

**Caller eager-load membership.current_tier:**

Cả 2 call site đều resolve `membership` trước khi gọi `_calculate_points`. Cần update query:

```python
# Trước (create_manual L.~85, _create_transaction_for_membership L.~350):
...
Nếu relationship `Membership.current_tier` chưa có → thêm vào `models/membership.py`:
```python
current_tier: Mapped["Tier | None"] = relationship(
    "Tier", foreign_keys=[current_tier_id], lazy="noload"
)
```

**Thứ tự earn vs recompute tier:** `TransactionService.create_manual` L.204 gọi `tier_svc.recompute_tier` SAU khi đã ghi `points_earned`. Tức `_calculate_points` đọc tier HIỆN TẠI (trước giao dịch) — đúng yêu cầu sản phẩm ("khách đang ở tier nào thì tính multiplier theo tier đó, không dùng tier mới sau khi vừa lên hạng").

**Redeem flow KHÔNG đổi** — chỉ edit earn path. Voucher redeem tính qua `VoucherService.redeem_voucher_atomic` có công thức riêng không đụng `_calculate_points`. Tier multiplier KHÔNG áp dụng cho redeem (quyết định đã chốt).

### 4.4. API contract

**Điểm xuất phát — endpoints đã có trong `backend/app/api/point_rules.py`:**
- `GET /partner/point-rules/active` — L.18, `require_owner_in_partner` (cần verify)
- `GET /partner/point-rules` — L.31, list
- `POST /partner/point-rules` — L.41, create
- **KHÔNG có `PATCH`** → phải ADD mới.

**ADD: `PATCH /partner/point-rules/{rule_id}` (Owner only):**
```
Request: { points_per_unit?: Decimal, unit_amount?: int,
           min_amount?: int, use_tiers?: bool, is_active?: bool }
Response 200: PointRuleResponse (include use_tiers field)
400: validate fail (vd points_per_unit ≤ 0, unit_amount ≤ 0)
403: staff role
404: rule không thuộc partner hiện tại
```

**UPDATE schema `PointRuleResponse` và `PointRuleCreate`:** thêm `use_tiers: bool = False` (default False — backward-compatible với rule cũ chưa có cột).

**ADD field `earn_multiplier` vào `TierCreateRequest` + `TierUpdateRequest` + `TierResponse`:**
```python
# backend/app/schemas/tier.py
class TierCreateRequest(BaseModel):
    name: str
    min_points: int
    earn_multiplier: Decimal = Field(default=Decimal("1.00"), ge=Decimal("0.50"), le=Decimal("5.00"), decimal_places=2)
    perks: dict | None = None

class TierUpdateRequest(BaseModel):
    name: str | None = None
    min_points: int | None = None
    earn_multiplier: Decimal | None = Field(default=None, ge=Decimal("0.50"), le=Decimal("5.00"), decimal_places=2)
    perks: dict | None = None
```

Endpoint `PATCH /partner/tiers/{tier_id}` (đã có) tự động nhận field mới qua schema update — không cần route change. Endpoint `GET /partner/tiers` list response cũng tự include qua `TierResponse`. Frontend TS type update tương ứng.
...
**Staff guard (2 lớp):**
- **Backend:** tất cả endpoint thuộc `/partner/point-rules/*` PATCH + `/partner/tiers/{id}` PATCH đã (hoặc sẽ) dùng `Depends(require_owner_in_partner)`. Verify từng route ở phase implement — nếu endpoint tồn tại đã dùng `require_staff_in_partner` phải đổi sang Owner. Staff gọi → 403 `"Chỉ chủ shop mới được cập nhật cấu hình tích điểm"`.
- **Frontend:** client-side guard trong page component — đọc `role` từ `partnerStore`, nếu ≠ `"owner"` thì `router.replace("/staff")` (trang POS mặc định của staff). Không dùng `/member/partners` vì staff user không có membership. Không dùng `/partner/dashboard` vì dashboard cũng staff-accessible — staff lạc vào `/partner/settings` sẽ loop. **Target redirect chốt: `/staff`.**
...
**Transaction detail sheet fields (read-only block):**
```
Thời gian tạo      : 24/04/2026 10:30
Nhân viên          : Nguyễn Văn An
Khách hàng         : Trần Thị Bình (+84 90xx)
Doanh thu          : 100,000 VND
Voucher            : GIAM20 (-15,000)
Thực thu           : 85,000 VND
Điểm đã tích       : 85 điểm
   ↳ (cấu hình hiện tại: 1 điểm/1,000 VND, hạng Gold × 1.50)
Phương thức        : Thủ công / QR
```

**Disclaimer điểm đã tích (I10):** dòng `(cấu hình hiện tại: ...)` phải ghi rõ là **cấu hình hiện tại** — KHÔNG phải cấu hình tại thời điểm giao dịch. Lý do: chúng ta KHÔNG snapshot `rule.points_per_unit` / `tier.earn_multiplier` vào transaction row (không mở rộng schema). Nếu owner đổi rule sau này, disclaimer tránh gây hiểu lầm "điểm cũ tính theo rule mới". Chỉ `points_earned` là số thật lưu vào DB; công thức hiển thị là **tham khảo**, không recalculate.
...
**Migration file count — CHỐT:**
- **2 revisions tổng cộng:**
  - Revision 1: `e1f2a3b4c5d6_drop_service_fee_infra.py` — cho Part A (drop tables + columns + check constraints). `down_revision = "162e25afc796"` (head hiện tại: rename_tenant_to_partner). `downgrade()` = no-op `pass`.
  - Revision 2: `e2a3b4c5d6e7_add_earn_rules_and_receipt_code.py` — gộp B+C (add `point_rules.use_tiers`, `tiers.earn_multiplier`, `transactions.receipt_code` + partial unique index). `down_revision = "e1f2a3b4c5d6"`. `downgrade()` revert đầy đủ (cột add được → drop được).

**Lý do gộp B+C vào 1 revision (không tách):** cả hai đều `op.add_column` / `op.create_index`, không conflict, cùng target table khác nhau (`point_rules`, `tiers`, `transactions`). 1 revision giảm 1 lần container restart khi deploy. Nếu cần rollback B hoặc C riêng sau này thì viết revision mới drop field cụ thể — không cần tách ngay bây giờ.
...
- `test_create_transaction_receipt_code_duplicate_409` — POS tạo mới 2 giao dịch cùng mã → giao dịch thứ 2 fail 409 (chặn TẠI ĐIỂM TẠO, không chỉ PATCH).
- `test_create_transaction_concurrent_same_receipt_code` — 2 async task `asyncio.gather(create_manual(..., receipt_code="DUP-001"), create_manual(..., receipt_code="DUP-001"))`. Expected: 1 success + 1 raise `DuplicateReceiptCodeError` (IntegrityError caught). Đảm bảo partial unique index ở DB level chặn race condition, KHÔNG chỉ dựa vào SELECT-THEN-INSERT trong service.
...
- Setup: partner, rule use_tiers=True, unit_amount=1000 VND → 1 point, tier Bronze min_points=0 mult=1.00, Gold min_points=500 mult=1.50.
- Create membership → earn 1000 VND → Bronze × 1.00 = 1 point. Assert `membership.total_points_earned = 1`.
- **Đẩy membership lên Gold:** set `membership.total_points_earned = 600` + commit, sau đó gọi `tier_svc.recompute_tier(partner_id, membership.id)` để `current_tier_id` được cập nhật sang Gold (KHÔNG set tay `current_tier_id` — để `recompute_tier` làm). Lý do: `recompute_tier` chọn tier dựa trên `min_points <= total_points_earned`, nên phải chạy đúng method thay vì patch field.
- Re-fetch membership (hoặc `await db.refresh(membership)`) để đảm bảo `current_tier` relationship load đúng Gold.
- Earn tiếp 10_000 VND rule 1pt/1000VND → base = 10, Gold × 1.50 = 15 points. Assert `txn.points_earned = 15` (test clean case không dính int truncate).
- Edge: `use_tiers=False` flow — set rule.use_tiers = False, earn 10_000 VND → 10 points (multiplier bỏ qua kể cả khi tier là Gold).
- Edge: earn 1000 VND với Gold (multiplier 1.50) → 1 × 1.50 = 1.5 → `int(1.5) = 1` (assert truncate hành vi Python). Document ở docstring test.
...
- Rollback test: `alembic downgrade -1` → revert add-columns (part B/C). Part A migration (drop service fee) có `downgrade()` là no-op `pass` — alembic downgrade qua revision này KHÔNG raise, KHÔNG restore schema. CI round-trip `upgrade → downgrade → upgrade` không crash. Document rõ ở commit message + spec section 3.1.
...
4. **Preflight count check TRƯỚC khi up backend (sanity để xác nhận an toàn drop):**
   ```bash
   ...
   "
   ```
   Kết quả môi trường prod hiện tại kỳ vọng: `fee_rows = 0`, `schedule_rows = 0`, `active_fee_campaigns = 0` (vì đồ án đã disable flag `SERVICE_FEE_ENABLED=False`). Nếu ≠ 0 → **DỪNG**, báo user xử lý data trước khi drop (nói "phải export hoặc report cho các campaign có phí trước khi mất dữ liệu").
5. Migration auto-run on backend startup (A revision + B+C revision).
6. `docker compose up -d backend frontend`.
7. Verify health: `curl https://loyalty.ecom-bill.com/api/health`.
8. Smoke test 5 flows section 8.3.
9. Monitor backend log 10 min, 0 5xx.
10. Reseed demo data: `docker exec loyalty-backend-prod python seed_demo.py` (seed idempotent).

Rollback plan:
- Nếu B+C migration fail: downgrade -1 revert B+C cleanly (cột add được drop được).
- Nếu A migration fail giữa chừng: `alembic downgrade` qua A là no-op `pass` → KHÔNG restore schema. Phải `pg_restore` từ backup bước 1. Document rõ ở migration header docstring + trong commit message.
