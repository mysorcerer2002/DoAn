...
class ReportBuilder:
    def __init__(self, title, subtitle="LUẬN VĂN TỐT NGHIỆP"):
        self.title = title
    ...
    def add_cover(self, sv="[HỌ TÊN SINH VIÊN]", mssv="[MSSV]",
                  gvhd="[HỌ TÊN GVHD]", year="[NĂM HỌC]"):
    ...
    def start_chapter(self, label, name):
        from docx.oxml.ns import qn
        self._chapter_idx += 1
        new_sec = self.doc.add_section(WD_SECTION.NEW_PAGE)
        sectPr = new_sec._sectPr
    ...
    def start_frontmatter(self, title):
        from docx.oxml.ns import qn
        new_sec = self.doc.add_section(WD_SECTION.NEW_PAGE)
        sectPr = new_sec._sectPr
        pb = sectPr.find(qn("w:pgBorders"))
        ...
        for p in new_sec.first_page_header.paragraphs:
            p.text = ""
        ...
        r = hp.add_run(title.upper())
        S._set_run_font(r, size=S.HEADER_FOOTER_SIZE, italic=True, bold=True)

        p_title = self.doc.add_paragraph(title.upper())
        S.style_toc_title(p_title)
    ...
    def bullet(self, text):
        ...
        if p.runs:
            S._set_run_font(p.runs[0], size=S.BODY_SIZE)
    ...
    def figure(self, image_path, caption, width_cm=14.0):
        ch = self._chapter_idx if self._chapter_idx > 0 else 0
        ...
        try:
            ...
            S._set_run_font(run, size=S.BODY_SIZE, italic=True)
        ...
        prefix = f"Hình {ch}-{idx}: " if ch > 0 else f"Hình A-{idx}: "
        num_run = p_cap.add_run(prefix)
    ...
    def table(self, headers, rows, caption=None):
        ...
        if caption:
            ...
            p = self.doc.add_paragraph()
            p.alignment = WD_ALIGN_PARAGRAPH.CENTER
            prefix = f"Bảng {ch}-{idx}: " if ch > 0 else f"Bảng A-{idx}: "
