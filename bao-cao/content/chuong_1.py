...
def build(rb) -> None:
    ...
    rb.p(
        ...
        "yêu cầu về tính nhất quán: khi đổi quà, hệ thống phải kiểm tra số dư trước "
        "khi trừ, và phải đảm bảo không có cách nào đưa số dư về âm dù có request "
        "đồng thời. Đề tài áp dụng ba lớp bảo vệ: kiểm tra số dư ở tầng service "
        "trước khi thực hiện giao dịch, ràng buộc CHECK points_balance >= 0 ở cấp "
        "cơ sở dữ liệu để chặn cứng bất kỳ UPDATE nào vi phạm, và cập nhật số dư "
        "trong cùng một transaction database để tránh race condition."
    )
