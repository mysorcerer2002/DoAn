...
def build(rb) -> None:
    ...
    rb.p(
        ...
        "với delta, reason, ref_type, ref_id và balance_after. Để chống "
        "race condition khi hai request đổi quà song song trên cùng một "
        "user, đề tài sử dụng pattern Optimistic Concurrency Control dựa "
        "trên database constraint thay vì SELECT FOR UPDATE thủ công: "
        "câu lệnh UPDATE users SET points_balance = points_balance - X "
        "WHERE id = U AND points_balance >= X RETURNING points_balance là "
        "atomic ở cấp PostgreSQL. Mệnh đề WHERE đóng vai trò chốt chặn "
        "race — request đến sau khi đụng row-level lock đợi commit của "
        "request trước, sau đó so điều kiện với số dư đã cập nhật và "
        "rowcount = 0 nếu không đủ; service kiểm tra RETURNING trả None "
        "thì raise InsufficientPointsError (HTTP 400). Lớp defense-in-depth "
        "thứ hai là CHECK constraint points_balance >= 0 ở cấp cơ sở dữ liệu, "
        "bắt mọi đường ghi khác không qua pattern atomic UPDATE WHERE — "
        "trong luồng đổi quà thông thường thì CHECK không kích hoạt vì WHERE "
        "đã loại trước. Trigger PostgreSQL no_update_or_delete_point_ledger "
        "được kích hoạt khi có lệnh UPDATE hoặc DELETE trên bảng point_ledger "
        "và tự động raise exception, ngăn mọi hành vi sửa lịch sử sau khi "
        "đã ghi."
    )
